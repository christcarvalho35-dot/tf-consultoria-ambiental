      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="px-4 py-3 text-left text-gray-600 font-medium">Thumbnail</th>
            ...
            {items.map((post) => (
              <tr key={post.id} className="hover:bg-gray-50">
                <td className="px-4 py-3">
                  {post.imagem_url
                    ? <Image src={post.imagem_url} alt={post.titulo} width={64} height={40} className="rounded-lg object-cover w-16 h-10" />
                    : <div className="w-16 h-10 rounded-lg bg-gray-100 flex items-center justify-center">✍️</div>}
                </td>
